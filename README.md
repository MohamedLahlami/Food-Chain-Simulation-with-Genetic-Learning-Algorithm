...
We took inspiration for this simulation from lab 8: EvolutionarySteeringBehavior, it uses the same algorthim to "teach" 3 types of animals to steer towards food and away from danger.
...
To make the code more modular and consistant, we used a unified dna structure with 6 parameters.
...
To get to a game state where a "reasonable" ecosystem can be sustained, we fixed the value for health loss to `0.002` per frame which comes up to `0.12` health lost per second for all animals, and experimented with different values for the initial health and the reproduction rate for each species.
initially, we wanted to make the mutation rate a class variable and tune it for each subclass but decided not to because the reproduction rate achieves the same effect (more or less) and is already a class variable.
